...
bool StmtParser::parse(unsigned verb, Stmt & s, std::string & error)
{
    ...
    switch (verb) {
        ...
        case 0x4B: {                                // BIN( — число в двоичные байты
            // BIN(<приёмник>[,2])=<выражение>: ни закрывающая скобка, ни знак
            // равенства не кодируются, как у PACK( и UNPACK(.
            s.kind = ST_BIN;
            s.bytes = 1;

            Expr target;
            // Заглядывать вперёд тут нельзя: у BIN(A¤)=J% за приёмником сразу
            // идёт индекс переменной (4B 02 22 3B), и заглядывание приняло бы
            // значение за список индексов. Массив это или скаляр — только по
            // таблицам.
            if (!ex_.parse_lvalue(target, true)) { ok = err(ex_.error()); break; }
            s.targets.push_back(target);

            // «,2» — пара DE DB. Смотрим сырые байты, а не лексемы: в позиции
            // операции индекс переменной источник читать откажется, а в
            // позиции операнда DE значит не запятую, а байтовую константу.
            // Позиция источника здесь верна: parse_lvalue заканчивает разбор
            // потреблением лексемы и заглядывания за собой не оставляет.
            const unsigned p = src_.pos();
            if (p + 1 < len_ && ops_[p] == 0xDE && ops_[p + 1] == 0xDB) {
                s.bytes = 2;
                src_.set_pos(p + 2);
            }
