...
// Определение статической константы, объявленной в классе. Без него
// собирается только сборка с оптимизацией: там компилятор подставляет
// значение прямо в место обращения. В отладочной `std::make_pair(DIRECT, 0u)`
// берёт её по ссылке (`interp.cpp`, `sf_call`), а это уже настоящее
// обращение к объекту, и компоновщик требует, чтобы объект где-то лежал.
// До C++17 объявления в классе для этого мало.
const unsigned Interp::DIRECT;
