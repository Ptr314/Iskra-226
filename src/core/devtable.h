...
enum SelectCode {
    ...
    // В текстовой форме читается однозначно, а какой код у него в потоке
    // токенов — неизвестно: `01` на эту роль лишь похож. Чтобы не выдумывать
    // кодировку, у текстовой формы свой признак.
    SC_TRIG   = 0xFE
};

// Единицы измерения углов; хранятся в SelectItem::addr у SC_TRIG.
