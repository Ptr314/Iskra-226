...
bool Interp::do_gcopy(Stream & st)
{
    ...
    if (!out) {
        // Устройства нет — это тот же вывод в никуда, что у блочного
        // обмена, и ключ `-i` его пропускает. `EDITOR` 1237 выкладывает
        // буфер на `/34`, где сидит своя микроЭВМ, а её микропрограмму
        // тот же ключ уже пропустил.
