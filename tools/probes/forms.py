...
FORMS = [
    ...
    ('COM CLEAR',                 'COM CLEAR без операнда'),
    ('COM CLEAR A()',             'COM CLEAR с массивом'),
    ('COM CLEAR A',               'COM CLEAR с переменной'),
    ...
    # --- графика
    ('PLOT <,,R>',                'PLOT с пустыми элементами'),
    ('PLOT <A,B,U>',              'PLOT с пером U'),
    ('PLOT <A,B,D>',              'PLOT с пером D'),
    ('PLOT <A,B,S>,<C,,C>',       'две группы, перо S и C'),
    ('PLOT <A+1,2*B,"X">',        'третий элемент — литерал'),
    ('PLOT <,,STR(A' + C + ',1,4)>', 'третий элемент — вырезка'),
    (C + 'OPEN A' + C + '()',      '¤OPEN'),
    (C + 'OPEN A' + C + '(),B' + C + '()', '¤OPEN с двумя буферами'),
    (C + 'OPEN ,B' + C + '()',     '¤OPEN с пропущенным первым'),
    ('NPLOT A' + C + '(),10,20',   'NPLOT'),
    ('DRAW A' + C + '(),10,20',    'DRAW'),
    ('DDRAW A' + C + '(),0,-3',    'DDRAW приращением'),
    ('DOT A' + C + '(),7,9',       'DOT'),
    ('LABEL A' + C + '(),,,"Y"',   'LABEL с пропущенными признаками'),
    ('LABEL A' + C + '()3,,,"Q"',  'LABEL с множителем размера'),
    ('WINDOW 0,559,0,255',         'WINDOW'),
    ('FRAME A' + C + '(),0,10,0,10', 'FRAME'),
    (C + 'MOVE A' + C + '(),I,J',  '¤MOVE'),
    ('TURN A' + C + '(),R,N,F',    'TURN'),
    ('STRETCH A' + C + '(),0,0,C,1/S', 'STRETCH'),
    (C + 'LET A' + C + '()=A' + C + '()', '¤LET'),
    (C + 'LET A' + C + '()=0',     '¤LET с обнулением'),
    (C + 'COPY /10,A' + C + '()',  '¤COPY на устройство'),
    # --- матричные
    ('MAT A=ZER',                 'MAT =ZER'),
    ('MAT A=B',                   'MAT присваивание'),
    ('MAT REDIM A(5)',            'MAT REDIM'),
    ('MAT PRINT A',               'MAT PRINT'),
    ('MAT PRINT A;',              'MAT PRINT в плотном формате'),
    ('MAT PRINT A,B',             'MAT PRINT списком'),
    ('MAT PRINT /0C,A',           'MAT PRINT на другое устройство'),
    ('MAT READ A',                'MAT READ'),
    ('MAT READ A' + C,            'MAT READ символьного массива'),
    ('MAT READ A,B(2,3)',         'MAT READ списком и с новой размерностью'),
    ('MAT INPUT A',               'MAT INPUT'),
    ('MAT INPUT A,B(2),C(2,4)',   'MAT INPUT списком'),
    ('MAT INPUT A' + C + '(3)8',  'MAT INPUT с длиной элемента'),
    ...
    ('A=SIN(X)',                  'SIN( — токен F9'),
    ('A=COS(X)',                  'COS( — токен FA'),
    ('A=TAN(X)',                  'TAN( — токен FB'),
    ('A=ARCSIN(X)',               'ARCSIN( — токен FC'),
    ('A=ARCCOS(X)',               'ARCCOS( — токен FD'),
    ('A=ARCTAN(X)',               'ARCTAN( — токен FE'),
    ('IF A=1XOR B=2THEN 30',      'связка XOR — токен E5'),
    ('SELECT D',                  'единицы углов: градусы'),
    ('SELECT R',                  'единицы углов: радианы'),
    ('SELECT G',                  'единицы углов: грады'),
    ('SELECT D,PRINT 0C',         'единицы углов вместе с группой'),
    ('MAT A()=CON',               'MAT CON — токен F0'),
    ('MAT A()=IDN',               'MAT IDN — токен EE'),
    ...
    ('LOAD DC F"ИМЯ"1000,,53',    'LOAD DC с пропущенным номером'),
    ...
    ('DATA SAVE DC CLOSE #2',     'закрытие строки таблицы'),
